...
def main():
    ...
    def preprocess_function(examples):
        ...
        second_sentences = [
            [f"{header} {context[context_idx]}" for context_idx in header] for i, header in enumerate(question_headers)
        ]
        # labels = [
        #     header.index(label) for i, (header, label) in enumerate(zip(question_headers, examples[label_column_name]))
        # ]
        ...
        para_list.append(examples['paragraphs'])
    ...
    # id_list = list(np.array([t for item in id_list for t in item]).reshape(-1, args.per_device_test_batch_size))
    id_list = [it for sublist in id_list for it in sublist]
    para_list = [it for sublist in para_list for it in sublist]
    ...
    #     train_dataset, shuffle=True, collate_fn=data_collator, batch_size=args.per_device_test_batch_size
    # )
    test_dataloader = DataLoader(test_dataset,\
                                collate_fn=data_collator, batch_size=args.per_device_test_batch_size, shuffle=False)
    ...
    #             min_loss = loss.item()
    #         if epoch > 0 and loss.item() < min_loss:
    #             output_dir = f"step_{completed_steps }"
    #             if args.output_dir is not None:
    #                 output_dir = os.path.join(args.output_dir, output_dir)
    #             accelerator.save_state(output_dir)
    ...
    for step, batch in enumerate(test_dataloader):
        ...
        with torch.no_grad():
            ...
            outputs = model(**batch)
            predictions = outputs.logits.argmax(dim=-1)
            # print('a', len(predictions)) # , len(id_list[step])
            # print(predictions)
            # pred.append(predictions)
            for p, i in zip(predictions, range(4)):
                # print(id_list[step*4+i])
                idx = id_list[step*4+i]
                pred_dict[idx] = para_list[step*4+i][p.item()]
                # print(step*4+i, '/', idl, ',', p.item(), para_list[step*4+i], para_list[step*4+i][p.item()])
    ...
    if args.output_dir is not None:
        with open(args.output_dir+"/mc_pred.csv", "w") as outfile:
